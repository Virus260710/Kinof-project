...
export default function AdminHelpCenter({ tickets, setTickets, notify }) {
  ...
  const advance = async (ticket) => {
    const next = ticket.status === "รอดำเนินการ" ? "กำลังดำเนินการ" : "เสร็จสิ้น";
    try {
      const updated = await updateProblemReportStatus(ticket.id, next);
    ...
    <div>
      ...
      <Card className="p-5 overflow-x-auto">
        <table className="w-full min-w-[760px] text-xs">
          ...
          <tbody>
            {shown.map((ticket) => <tr key={ticket.id} className="border-b border-gray-50 align-top">
              <td className="py-3 text-gray-700">{ticket.user?.username}<br /><span className="text-gray-400">{ticket.user?.email}</span></td>
              <td className="py-3 text-gray-700">{ticket.title}</td>
              <td className="py-3 text-gray-500 max-w-xs">{ticket.detail}</td>
              <td className="py-3 text-gray-500">{ticket.images?.length ?? 0} รูป</td>
              <td className="py-3"><Pill tone={toneFor(ticket.status)}>{ticket.status}</Pill></td>
              <td className="py-3 flex gap-2"><Button variant="secondary" size="sm" onClick={() => setSelected(ticket)}>ดูรายละเอียด</Button>{ticket.status !== "เสร็จสิ้น" && <button onClick={() => advance(ticket)} className="text-xs text-white rounded-lg px-3 py-1.5" style={{ background: NAVY }}>{ticket.status === "รอดำเนินการ" ? "รับเรื่อง" : "ทำเครื่องหมายเสร็จสิ้น"}</button>}</td>
            </tr>)}
      ...
      {selected && <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center p-4" role="dialog" aria-modal="true">
        <Card className="w-full max-w-2xl max-h-[90vh] overflow-y-auto p-6">
          ...
          <div className="mt-5 space-y-3 text-sm"><div><b>หัวข้อ:</b> {selected.title}</div><div><b>รายละเอียด:</b><p className="mt-1 whitespace-pre-line text-gray-600">{selected.detail}</p></div><div><b>สถานะ:</b> <Pill tone={toneFor(selected.status)}>{selected.status}</Pill></div><div className="text-xs text-gray-500">ส่งเมื่อ {new Date(selected.createdAt).toLocaleString("th-TH")}</div></div>
