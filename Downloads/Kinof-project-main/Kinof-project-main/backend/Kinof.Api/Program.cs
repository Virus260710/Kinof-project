    ...
    CancellationToken cancellationToken) =>
{
    ...
    return await service.UpdateStatusAsync(reportId, payload?.Status, payload?.AdminComment, cancellationToken);
});
problemReports.MapPatch("/{reportId:guid}/staff-note", async (
    Guid reportId,
    HttpRequest request,
    ClaimsPrincipal user,
    ProblemReportService service,
...
{
    if (!IsStaff(user)) return Results.Forbid();
    var payload = await request.ReadFromJsonAsync<UpdateProblemReportStaffNoteRequest>(cancellationToken);
    return await service.UpdateStaffNoteAsync(reportId, payload?.StaffNote, cancellationToken);
});
...
public sealed record UpdateProblemReportStatusRequest(string Status, string? AdminComment);
public sealed record UpdateProblemReportStaffNoteRequest(string? StaffNote);
