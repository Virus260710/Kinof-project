...
public sealed class ProblemReportService(AppDbContext db, IWebHostEnvironment environment)
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (adminComment?.Length > 2000)
            return Results.BadRequest(new { message = "ข้อความตอบกลับยาวเกิน 2,000 ตัวอักษร" });
    ...
    public async Task<IResult> UpdateStaffNoteAsync(Guid reportId, string? staffNote, CancellationToken cancellationToken)
    {
        staffNote = staffNote?.Trim();
        if (staffNote?.Length > 2000)
            return Results.BadRequest(new { message = "หมายเหตุจากเจ้าหน้าที่ยาวเกิน 2,000 ตัวอักษร" });

        var report = await db.ProblemReports.SingleOrDefaultAsync(x => x.Id == reportId, cancellationToken);
        if (report is null) return Results.NotFound(new { message = "ไม่พบคำร้องนี้" });

        report.StaffNote = string.IsNullOrWhiteSpace(staffNote) ? null : staffNote;
        report.UpdatedAt = DateTime.UtcNow;
        await db.SaveChangesAsync(cancellationToken);
        return Results.Ok(await ToResponseAsync(reportId, null, true, cancellationToken));
    }
